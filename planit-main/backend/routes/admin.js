...
// Helper: parse datetime-local string as local time (not UTC)
// datetime-local inputs return "YYYY-MM-DDTHH:mm" without timezone
// new Date() treats this as UTC, but we want local time interpretation
  ...
  try {
    ...
    if (action === 'activate' || action === 'schedule') {
      ...
      const rec = await Mnt.create({
        ...
        eta:   eta   ? parseLocalDateTime(eta)   : null,
        start: start ? parseLocalDateTime(start) : (action === 'activate' ? new Date() : null),
